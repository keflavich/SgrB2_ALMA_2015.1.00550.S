...
min_nchans = 1900
frange = {0: [218736475830, 218794096600],
...
def make_spw_cube(spw='spw{0}', spwnum=0, fntemplate='SgrB2N',
    ...
    header_fn = glob.glob('sgr_b2m.{5}.{0}.lines0-{4}.clarkclean1000.{3}'
                          .format(spw, fntemplate, fnsuffix, filesuffix,
                                  first_endchannel, source))
    if len(header_fn) != 1:
    ...
    fileglob = "sgr_b2m.{4}.{0}.lines*{3}".format(spw, fntemplate, fnsuffix,
                                                  filesuffix, source)
...
if __name__ == "__main__":
    ...
    for spw in (0,):#1,2,3):
        ...
        if os.path.exists('sgr_b2m.N.spw{0}.lines0-60.clarkclean1000.image.pbcor.fits'.format(spw, robust)):

            make_spw_cube(spw='spw{0}', spwnum=spw,
                          fntemplate='full_SgrB2N',
                          source='N',
                          overwrite_existing=False, bmaj_limits=None,
                          fnsuffix="", filesuffix='image.pbcor.fits',
                          first_endchannel=60,
        ...
        if os.path.exists('sgr_b2m.M.spw{0}.lines0-60.clarkclean1000.image.pbcor.fits'.format(spw, robust)):
            make_spw_cube(spw='spw{0}', spwnum=spw,
                          fntemplate='full_SgrB2M',
                          source='M',
                          overwrite_existing=False, bmaj_limits=None,
                          fnsuffix="", filesuffix='image.pbcor.fits',
                          first_endchannel=60,
                          #debug_mode=True,
                          cropends=0, minimize=True, add_beam_info=True)
        #elif os.path.exists('sgr_b2m.N.spw{0}.channels0to373.image.pbcor.fits'.format(spw, robust)):
