...
WORK_DIR='/lustre/aginsbur/sgrb2/2016.1.00550.S/imaging/'
cd ${WORK_DIR}
REDUCTION_DIR='/lustre/aginsbur/sgrb2/2016.1.00550.S/reduction/band3/'
...
export CASACMD="execfile('$REDUCTION_DIR/scriptForImaging_lines_B3_outliers.py')"
echo $CASACMD
echo $PBS_NODEFILE

#mpicasa -machinefile $PBS_NODEFILE casa -quiet --nogui --nologger --log2term -c "${CASACMD}"
mpicasa -n 8 casa -quiet --nogui --nologger --log2term -c "${CASACMD}"
#xvfb-run -d casa --nogui --nologger -c "${CASACMD}"


# casa's python requires a DISPLAY for matplot so create a virtual X server
#xvfb-run -d casa --nogui --nologger -c "execfile('$REDUCTION_DIR/scriptForImaging_lines_zoom.py')"
